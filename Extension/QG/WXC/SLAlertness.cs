...
public class SLAlertness : SpaceTimeWitchCards, IAbnormalityCard
{
    ...
    [
        new HoverTip(
            ...
            new LocString("cards", "SL_ALERTNESS")
        ),
    ];

    public async Task ApplyEffect(PlayerChoiceContext ctx, Player player)
    {
        await PowerCmd.Apply<SLAlertnessP>(ctx, player.Creature, 1, player.Creature, this);
    }
