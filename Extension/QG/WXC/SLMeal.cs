...
public class SLMeal : SpaceTimeWitchCards, IAbnormalityCard
{
    ...
    [
        new HoverTip(
            ...
            new LocString("cards", "SL_MEAL")
        ),
    ];

    public async Task ApplyEffect(PlayerChoiceContext ctx, Player player)
    {
        await PowerCmd.Apply<SLMealP>(ctx, player.Creature, 1, player.Creature, this);
    }
