...
public class SLAxe : SpaceTimeWitchCards, IAbnormalityCard
{
    ...
    [
        new HoverTip(
            ...
            new LocString("cards", "SL_AXE")
        ),
    ];

    public async Task ApplyEffect(PlayerChoiceContext ctx, Player player)
    {
        await PowerCmd.Apply<SLAxeP>(ctx, player.Creature, 1, player.Creature, this);
    }
