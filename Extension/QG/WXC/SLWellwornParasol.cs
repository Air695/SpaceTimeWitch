...
public class SLWellwornParasol : SpaceTimeWitchCards, IAbnormalityCard
{
    ...
    [
        new HoverTip(
            ...
            new LocString("cards", "SL_WELLWORN_PARASOL")
        ),
    ];

    public async Task ApplyEffect(PlayerChoiceContext ctx, Player player)
    {
        await PowerCmd.Apply<SLWellwornParasolP>(ctx, player.Creature, 1, player.Creature, this);
    }
