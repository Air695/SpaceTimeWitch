...
public class SLCocoon : SpaceTimeWitchCards, IAbnormalityCard
{
    ...
    [
        new HoverTip(
            ...
            new LocString("cards", "SL_COCOON")
        ),
    ];

    public async Task ApplyEffect(PlayerChoiceContext ctx, Player player)
    {
        await PowerCmd.Apply<SLCocoonP>(ctx, player.Creature, 1, player.Creature, this);
    }
