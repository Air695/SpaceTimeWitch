...
public class SLGlitter : SpaceTimeWitchCards, IAbnormalityCard
{
    ...
    [
        new HoverTip(
            ...
            new LocString("cards", "SL_GLITTER")
        ),
    ];

    public async Task ApplyEffect(PlayerChoiceContext ctx, Player player)
    {
        await PowerCmd.Apply<SLGlitterP>(ctx, player.Creature, 1, player.Creature, this);
    }
