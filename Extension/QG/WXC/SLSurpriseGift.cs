...
public class SLSurpriseGift : SpaceTimeWitchCards, IAbnormalityCard
{
    ...
    [
        new HoverTip(
            ...
            new LocString("cards", "SL_SURPRISE_GIFT")
        ),
    ];

    public async Task ApplyEffect(PlayerChoiceContext ctx, Player player)
    {
        await PowerCmd.Apply<SLSurpriseGiftP>(ctx, player.Creature, 1, player.Creature, this);
    }
